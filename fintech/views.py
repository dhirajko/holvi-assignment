...
class SelectedUserDetails(APIView):
    def get(self, request, user_id, *args, **kwargs):
        ...
        if not request.user.is_staff:
            return Response(data={'msg':'Unauthorized user'},status=401)
...
class UserAccount(APIView):
    def get(self, request, user_id, *args, **kwargs):
        ...
        if not (request.user.is_staff or request.user.id == int(user_id)):
            return Response(data={'msg':'Unauthorized user'},status=401)
        serialized_accounts = account_utility.account_by_user_id(user_id)
        return Response(serialized_accounts)
...
class WithdrawView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if new_balance <= 0:
            return Response(data={'msg':'Not enough balance to withdraw'},status=412)
